...
def register():
    ...
    if form.validate_on_submit():
        ...
        # if user exists in database then flash message and say try another username
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.verified and user.enabled  and bcrypt.check_password_hash(user.password, form.password.data):
            login_user(user, remember=form.remember.data)
            flash("Login in successful", "success")
            next_page = request.args.get('next')
            return redirect(url_for('user.choosefile'))
        elif user and  not user.verified:
            flash("You are not Verified yet","danger")
        elif user and not user.enabled:
            flash("You have been disabled","danger")
        else:
